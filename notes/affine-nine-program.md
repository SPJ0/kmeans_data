...
* **Lifting ⟺ vertex parity.**  Over every slope field enumerated
  (480 across two test triangulations, plus every field induced by an
  actual representation), the F_2 system is solvable exactly when a
  local parity — the XOR, over the faces around each vertex, of the
  pinned bits of the two spokes — vanishes at every vertex.  This is
  the expected cohomological statement (the dependencies of the face
  constraints on a sphere are generated by vertex stars) and should be
  provable as a lemma; it is *not* yet proved here.

**Consequence (modulo the parity lemma).**  Conjecture A — hence the
